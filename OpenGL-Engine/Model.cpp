...
unsigned int loadTexture(const char* path)
{
    ...
    {
        std::cout << "Texture failed to generate..." << std::endl;
        stbi_image_free(data);
    }
    return textureID;
}
    ...
    for (unsigned int i = 0; i < faces.size(); i++)
    {
        ...
        if (data)
        {
            ...
            if (nrChannels == 1 || nrChannels == 2)
                ...
                format = GL_RGBA;
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_SRGB_ALPHA, width, height, 0, format, GL_UNSIGNED_BYTE, data);        
            stbi_image_free(data);
        }
        else
        {
...
{
    std::cout << "we're on the node " << node->mName.C_Str() << std::endl;
...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
    ...
    {
        std::cout << iter.type << ": " << iter.path << std::endl;
    }
